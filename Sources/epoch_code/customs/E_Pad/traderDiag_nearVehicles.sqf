...
_radius spawn {
	...
	if (count _vehs < 1) then {
		...
		_vehs spawn {
			{
				[player, position _x, "ICON", "EpochBG_Diamond", "ColorBlue", [0.7,0.7], "", 0, getText (configFile >> "CfgVehicles" >> (typeOf _x) >> "displayName"), 0.8,format["vehMarker%1",_forEachIndex]] call EPOCH_makeMarker;
			} forEach _this;
			sleep (10 + random 10);
			{
				[format["vehMarker%1",_forEachIndex]] call EPOCH_removeMarker;
			} forEach _this;
		};
	};
