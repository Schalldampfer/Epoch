...
// Time based
	...
	timeDifference 		= -6; 			// Server uses real time this will allow you to offset just the hour.
...
// Events
	...
	events[] = {
		...
		{ 2400, "ContainerSpawner", 0 , 2, -1, {} ,{"VR"}},
		{ 1440, "GardenManager", 0 , 2, -1, {} ,{"VR"}},
		{ 1440, "SolarChargerManager", 0 , 2, -1, {} ,{"VR"}},
		{ 2700, "MoneyDrop", 0 , 2 , -1, {} ,{"VR"}},
		{ 2400, "PlantSpawner", 0 , 2 , -1, {} ,{"VR"}} //No comma on last Entry
	};

// Antagonists
	antagonistChancePDeath = 0.0; 		//33% chance when player was killed from a other player (selfkill doesn't count)

// Player Related
	cloneCost = 0; 					// debt incurred on player death
	MaxBankDebitforTrade = 0;		// If Player has less money on Bank, Crypto from Trade goes directly to Bank instead to Player
...
// Traders
	taxRate = 0.6; 						// controls the price increase for purchases
/*
...
*/
	...
	StaticTraderItemPurge[] = {150,15};	// {ItemCount,ReducePercent} - If a static trader have more than x different items, on restart the items will be reduced by y percent
...
// Spawntables
	forcedVehicleSpawnTable = "allowedVehiclesList"; 		// leave blank for default. Options: "allowedVehiclesList","allowedVehiclesList_CUP","allowedVehiclesList_MAD","allowedVehiclesList_MADCUP"
	forcedLootSpawnTable = "CfgLootTable"; 			// leave blank for default. Options: "CfgLootTable","CfgLootTable_CUP","CfgLootTable_MAD","CfgLootTable_MADCUP"

// Markers
	showEarthQuakes = "false"; 			// show mineral viens caused by earthquakes
